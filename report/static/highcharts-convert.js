...
(function () {
	...
	mapCLArguments = function () {
		...
		if (system.args.length < 1) {
			console.log('Commandline Usage: highcharts-convert.js -infile URL -inData data -outfile filename -scale 2.5 -width 300 -constr Chart -callback callback.js');
		...
		for (i = 0; i < system.args.length; i += 1) {
			if (system.args[i].charAt(0) === '-') {
				...
				if (key === 'infile' || key === 'inData' || key === 'callback' || key === 'dataoptions' || key === 'globaloptions' || key === 'customcode') {
	...
	render = function (params, exitCallback) {
		...
		if (params.length < 1) {
			...
			input = params.infile || params.inData;
