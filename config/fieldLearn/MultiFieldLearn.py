...
class MPositionalRule:
    ...
    def getJavaRegexStr(self):
        return str("".join(self._pattern)).replace("(?P<", "(?<")


class JavaRegexRule:
    def __init__(self, javaPatterns):
        self._javaPatterns = javaPatterns
        self._pythonPatterns = [str("".join(x).replace("(?<", "(?P<")) for x in javaPatterns]
        self._regexes = [re.compile(x) for x in self._pythonPatterns]
        self._raw = '_raw'

    def __str__(self):
        return "regex: %s" % (str("".join(self._pythonPatterns)))

    def findExtractions(self, markedEvent):
        if "_event" in markedEvent:
            markedEvent = markedEvent["_event"]
        raw = markedEvent[self._raw]
...
def test_filter():
    ...
    receiveJava = [x.getJavaRegexStr() for x in learn("_raw", MarkedEvents, [], [], "HTTP")] # filter is HTTP
...
#    if not check_test_results(expect,receive):
    if len(receive) == 0:
        ...
        print "test_filter passed: rule = ", receiveJava[0]
...
def test_filename_extraction():
    ...
    MarkedEvents = [
            ...
            {
                '_event': {'_raw': '[Thus May 11 16:56:12 2014] [error] [client 10.1.1.140] File does not exist: /usr/local/apache/htdocs/favicon.ico'},
                'riqi': [1,5]
            }
    ...
    events = [
        {
            '_raw': '[Wed May 11 16:56:11 2014] [error] [client 10.1.1.140] File does not exist: /usr/local/apache/ComBeta/images/page_bg.gi'
        },
        {
            '_raw': '[Wed May 11 16:56:12 2014] [error] [client 10.1.1.140] File does not exist: /usr/local/apache/htdocs/favicon.ico'
        },
        {
            '_raw': '[Wed May 11 16:56:12 2014] [client 10.1.1.140] File does not exist: 200 biubiubiu'
        }
    ]
    expect = ['(?:[^/\\n]*/){5}(?P<myfield>\w+\.\w+)']
    # receive = [x.getPattern() for x in learn("_raw", MarkedEvents, events, [], "")]
    rules = learn("_raw", MarkedEvents, [], [], "")
#    if not check_test_results(expect,receive):
    if len(rules) == 0:
...
def test_java_extract():
    print "running java regex extract"
    expectRegex = ['(?<begin>\d+)', 'begin(?<end>\d+)']
    markedEvent1 = {
            '_event': { "_raw": "123" }
            }
    markedEvent2 = {
            '_event': { "_raw": "begin123" }
            }
    markedEvent3 = {
            '_event': { "_raw": "not match" }
            }

    rule = JavaRegexRule(expectRegex)
    result1 = rule.findExtractions(markedEvent1)
    result2 = rule.findExtractions(markedEvent2)
    result3 = rule.findExtractions(markedEvent3)
    print result1
    print result2
    print result3
