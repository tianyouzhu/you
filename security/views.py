    ...
    if is_login:
        ...
        if res["results"]["enable_security"]:
            ...
            return render(request, 'security/overview.html',
                          {"active": "security", "subnav": "overview", "user": is_login["u"], "email": is_login["e"],
...
def attack_details(request, **kwargs):
    my_auth = MyBasicAuthentication()
    is_login = my_auth.is_authenticated(request, **kwargs)
    if is_login:
        res = BackendRequest.get_func_auth({
        ...
        if res["results"]["enable_security"]:
            with_sg = check_with_sourcegroup(is_login)
            cf_per = check_with_permission(is_login)
            return render(request, 'security/attack_details.html',
                          {"active": "security", "subnav": "attack_details", "user": is_login["u"], "email": is_login["e"],
                           "token": is_login["t"], "userid": is_login["i"],
                           "role": is_login["r"], "with_sg": with_sg,
                           "cf_per": cf_per})
        else:
...
def config(request, **kwargs):
    ...
    if is_login:
        ...
        if res["results"]["enable_security"]:
            with_sg = check_with_sourcegroup(is_login)
            cf_per = check_with_permission(is_login)
            return render(request, 'security/config.html',
                          {"active": "source", "subnav": "security", "user": is_login["u"], "email": is_login["e"],
                           "token": is_login["t"], "userid": is_login["i"],
                           "role": is_login["r"], "with_sg": with_sg,
                           "cf_per": cf_per})
        else:
